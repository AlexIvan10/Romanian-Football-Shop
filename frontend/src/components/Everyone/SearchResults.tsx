...
function SearchResults(): JSX.Element {
    ...
    const fetchProducts = async () => {
        try {
            setLoading(true);
            const response = await fetch(`http://localhost:8080/api/product/searchByName?name=${encodeURIComponent(query)}`);
            if (!response.ok) {
        ...
        <Box sx={{
            ...
            >
                <Container>
                    <Box sx={{ display: 'flex', justifyContent: 'center', py: 1, gap: 1 }}>
                        <NavButton onClick={() => navigate('/all-products')}>
                            All products
                        </NavButton>
            ...
            <Container sx={{ mt: 4, mb: 4 }}>
                ...
                >
                    ...
                    {/* Filter Controls - Only show if there are products */}
